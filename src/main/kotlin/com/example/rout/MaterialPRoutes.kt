...
fun Route.materialPRoutes(dao: DAOFacadeImpl) {
    route("/materialP") {
        ...
        get {
            call.respond(HttpStatusCode.OK, "Ruta base de Material Peligroso. Usa /crear, /obtener o /{idMaterialP} para más acciones.")
        }

        // Obtener todos los materiales peligrosos (GET /materialP/obtener)
        get("/obtener") {
            try {
                val materialesP = dao.allMaterialesP() // Método DAO para obtener todos los materiales peligrosos
                call.respond(HttpStatusCode.OK, materialesP)
            } catch (e: Exception) {
                call.respond(HttpStatusCode.InternalServerError, mapOf("error" to "Error al obtener materiales peligrosos: ${e.message}"))
            }
        }
        ...
        post("/crear") {
            try {
                ...
                call.respond(HttpStatusCode.InternalServerError, mapOf("error" to "Error al crear material peligroso: ${e.message}"))
            }
        }

        // Obtener un material peligroso por ID (GET /materialP/{idMaterialP})
        get("/{idMaterialP}") {
            ...
            try {
                ...
                call.respond(HttpStatusCode.InternalServerError, mapOf("error" to "Error al obtener material peligroso: ${e.message}"))
            }
        }
        ...
        put("/actualizar/{idMaterialP}") {
            ...
            try {
                ...
                call.respond(HttpStatusCode.InternalServerError, mapOf("error" to "Error al actualizar material peligroso: ${e.message}"))
            }
        }
        ...
        delete("/eliminar/{idMaterialP}") {
            ...
            try {
                ...
                call.respond(HttpStatusCode.InternalServerError, mapOf("error" to "Error al eliminar material peligroso: ${e.message}"))
            }
        }
    }
}
